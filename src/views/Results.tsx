...
const Results: React.FC = () => {
  ...
  const [turnoutData, setTurnoutData] = React.useState({
    total_eligible: 0,
    votes_cast: 0,
  ...
  React.useEffect(() => {
    const fetchTurnout = async () => {
      try {
        ...
        console.error('Failed to connect to Render backend endpoint:', err);
      }
    };
    ...
    // Live polling: refresh metrics every 10 seconds
